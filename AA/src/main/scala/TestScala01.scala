...
object TestScala01 {
  ...
  def getConnection: Connection = {
    def main(args: Array[String]): Unit = {
      ...
      //临时文件保存位置，在hdfs上
      print(" 临时文件保存位置 hdfs://bigbang-180:8020/user/ss/test2")
      val tmpdir = "hdfs://bigbang-180:8020/user/ss/test2"

      val hconf = new Configuration()
      hconf.set("fs.defaultFS", "hdfs://bigbang-180:8020")

      val fs = FileSystem.get(new URI("hdfs://bigbang-180:8020"), hconf, "hdfs") //hadoop为你的服务器用户名
      ...
      conf.set("hbase.mapreduce.hfileoutputformat.table.name", "test111")
      //此处运行完成之后,在tmpdir生成的Hfile文件
      rdd.saveAsNewAPIHadoopFile(tmpdir, classOf[ImmutableBytesWritable], classOf[KeyValue], classOf[HFileOutputFormat2], conf)
      //开始HFile导入到Hbase
